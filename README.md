...
打开 http://127.0.0.1:8765 。端口占用时加 `--port 8766`。

1. 输入 `我家有鸡蛋4个，番茄2个，油100克，盐20克`，核对后确认保存。
2. 输入 `给2个人做饭`，查看菜谱、步骤及每人营养估算。
3. 做完点击扣减按钮，只看推荐不会消耗库存。
4. 重启后输入 `我想做饭`，读取已有库存和人数。
5. 输入 `补货鸡蛋6个` 并确认，库存累加。盘点设置所列食材余量，未列出者保留。

记忆保存在 `data/pantry.sqlite3`，Git已忽略。仅本机单家庭使用，不可直接暴露公网。

首版为规则式中文对话，10种食材、8道菜，未知食材或数量会提示。营养是未逐条核验的演示近似值。未实现健康、过敏、保质期约束及整餐组合优化；下文Skill说明不表示网页已实现所有能力。

[框架与项目调研](docs/architecture.md)。测试：`python -m unittest discover -s tests -v`。
